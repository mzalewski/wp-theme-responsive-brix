		...
		for ( $i=0; $i < $columns; $i++ ) :
			if ( isset( $alphas[ $i ] ) ) :
				hoot_register_sidebar(
					array(
						'id'          => 'footer-' . $alphas[ $i ],
						'name'        => sprintf( _x( 'Footer %s', 'sidebar', 'hoot' ), strtoupper( $alphas[ $i ] ) ),
						'description' => __( 'You can set footer columns in Theme Options page.', 'hoot' )
					)
				);
			endif;
		endfor;
...
function hoot_widgetized_template_register_sidebars() {
	...
	if ( current_theme_supports( 'hoot-widgetized-template' ) ) :
		...
		$defaults = array(
			'area_a' => __('Widgetized Template - Area A', 'hoot'),
			'area_b' => __('Widgetized Template - Area B', 'hoot'),
		...
		foreach ( $defaults as $key => $name ) {
			hoot_register_sidebar(
				array(
					...
					'name'        => $name,
					'description' => __( 'You can order Widgetized Template areas in Theme Options page.', 'hoot' )
				)
			);
		}
