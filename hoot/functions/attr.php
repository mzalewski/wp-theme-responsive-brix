...
function hoot_attr_sidebar( $attr, $context ) {
	...
	if ( !empty( $context ) ) {
		...
		$attr['aria-label'] = esc_attr( sprintf( _x( '%s Sidebar', 'sidebar aria label', 'hoot' ), hoot_get_sidebar_name( $context ) ) );
	}
...
function hoot_attr_menu( $attr, $context ) {
	...
	if ( !empty( $context ) ) {
		...
		$attr['aria-label'] = esc_attr( sprintf( _x( '%s Menu', 'nav menu aria label', 'hoot' ), hoot_get_menu_location_name( $context ) ) );
	}
...
function hoot_attr_entry_published( $attr ) {
	...
	$attr['title']    = get_the_time( _x( 'l, F j, Y, g:i a', 'post time format', 'hoot' ) );
...
function hoot_attr_comment_published( $attr ) {
	...
	$attr['title']    = get_comment_time( _x( 'l, F j, Y, g:i a', 'comment time format', 'hoot' ) );
